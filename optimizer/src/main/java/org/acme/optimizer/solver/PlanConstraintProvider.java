package org.acme.optimizer.solver;

import ai.timefold.solver.core.api.score.buildin.hardsoft.HardSoftScore;
import org.acme.optimizer.domain.Activity;
import ai.timefold.solver.core.api.score.stream.Constraint;
import ai.timefold.solver.core.api.score.stream.ConstraintFactory;
import ai.timefold.solver.core.api.score.stream.ConstraintProvider;
import org.acme.optimizer.domain.Resource;
...
public class PlanConstraintProvider implements ConstraintProvider {
    ...
    public Constraint[] defineConstraints(ConstraintFactory constraintFactory) {
        return new Constraint[] {
                // Hard constraints
                producibleItem(constraintFactory),
...
//                leastStockKeepingDay(constraintFactory)
    ...
    Constraint producibleItem(ConstraintFactory constraintFactory) {
        // An item can be produced using specific resource
        return constraintFactory
                .forEach(Activity.class)
                .filter(activity -> !activity.getResource().getItemList().contains(activity.getItem()))
                .penalize(HardSoftScore.ONE_HARD)
                .asConstraint("Producible item");
    }

    Constraint resourceCapacity(ConstraintFactory constraintFactory) {
        // A resource has capacity limitation
        return constraintFactory
                .forEach(Activity.class)
                .join(Resource.class, equal(Activity::getResourceName, Resource::getName))
                .groupBy((activity, resource) -> activity.getTimeBucket(), (activity, resource) -> resource, sumLong((activity, resource) -> activity.getCapacity()))
